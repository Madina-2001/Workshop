...
from configuration import confi_folder
...
def print_help_string():
    help_string = r''''pwf' -- Печать рабочей папки
'mkfld' -- Создание папки
'mkfld' -- Сделать папку рекурсивной
'rmfld' -- Удалить папку
'rmflds' -- Удалить рекурсивную папку
'gtf' -- Перейти в папку (изменит текущую папку)
'crtfl' -- Создание файла
'wrtfl' -- Запись в файл (потоковый ввод строки)
'rdfl' -- Отобраить файл
'rmfl' -- Удалить файл
'cpflfd' -- Копировать file_name в папку/new_file
'rpls' -- Заменить файл или папку на другие файл или папку
'rnm' -- Переименовать файл или папку
'exit' -- Выход
...
current_directory = [confi_folder]
command_prompt()
